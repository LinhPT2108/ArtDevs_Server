...
public class JwtTokenProvider {
	...
	public String generateToken(User user, Collection<SimpleGrantedAuthority> authorities) {
		...
		long expirationTime = System.currentTimeMillis() + thirtyDaysInMillis;
		Algorithm algorithm = Algorithm.HMAC256(secret_key.getBytes());
		return JWT.create().withSubject(user.getUsername())
				.withExpiresAt(new Date(expirationTime))
				.withClaim("role", user.getRole().getRoleName()).sign(algorithm);
	}

	public String generateRefeshToken(User user, Collection<SimpleGrantedAuthority> authorities) {
		long thirtyDaysInMillis = 31L * 24 * 60 * 60 * 1000; // 30 days in milliseconds
		long expirationTime = System.currentTimeMillis() + thirtyDaysInMillis;
		Algorithm algorithm = Algorithm.HMAC256(secret_key.getBytes());
		return JWT.create().withSubject(user.getUsername())
	...
	public boolean validateToken(String authToken) {
		try {
			Algorithm algorithm = Algorithm.HMAC256(secret_key.getBytes());
			JWTVerifier verifier = JWT.require(algorithm).withSubject("art-backend").build();
	...
	public void deleteToken(String authToken) {
		try {
			...
			String invalidatedToken = JWT.create().withSubject(decodedJWT.getSubject()).withExpiresAt(new Date())
					.sign(invalidateAlgorithm);

			// In ra để kiểm tra, bạn có thể thay thế bằng cách khác như lưu vào một danh
			// sách token đã hết hạn
