...
public class UserServiceImpl implements UserService {
	...
	@Autowired
	TransitioninfoRepository traninfoservice;
	...
	// Gợi ý bạn bè theo demand
	@Override
	public List<User> findUserDemand() {

		Authentication auth = SecurityContextHolder.getContext().getAuthentication();
		User loggedInUser = findByEmail(auth.getName());

		if (loggedInUser == null) {
			// Xử lý trường hợp người dùng hiện tại không được đăng nhập hoặc không tồn tại
		...
		if (userRole == null) {
			// Xử lý trường hợp không tìm thấy vai trò
			return Collections.emptyList();
		}

		List<User> result = new ArrayList<>();

		for (User user : userRepository.findByRole(userRole)) {
			List<ProgramingLanguage> skillsOfUser = user.getUserSkill().stream()
					.map(Skill::getLanguage)
					.collect(Collectors.toList());

			List<ProgramingLanguage> demandOfLoggedInUser = loggedInUser.getUserDemand().stream()
					.map(Demand::getLanguage)
					.collect(Collectors.toList());

			if (!Collections.disjoint(skillsOfUser, demandOfLoggedInUser)) {
				// Nếu có ít nhất một kỹ năng chung giữa yêu cầu của người dùng hiện tại và kỹ
				// năng của người dùng khác
				result.add(user);
			}
		}
	...
	public Boolean SendMatchMentor(String mentorID) {
		...
		if (walletUserLogin.getSurplus() >= mentor.getMatchPrice() && mentor.getRole().getId() == 3) {
			...
			if (relationrepository.findByUserOneIdAndUserTwoIdAndStatus(userlogin, mentor, 2) == null
				...
				RelationShip relation = relationrepository.findRelationshipWithFriendWithStatus(userlogin.getUserId(),
						mentor.getUserId(), 3);
				relation.setStatus(2);
				relationrepository.save(relation);
			}
	...
	public Boolean AcceptMatchMentor(String userid) {
		...
		if (mentorlogin.getRole().getId() == 3) {
			...
			// Save MQH với mentor
			RelationShip matchbyuser = relationrepository.findRelationshipWithFriendWithStatus(user.getUserId(),
					mentorlogin.getUserId(), 2);
			if (matchbyuser == null) {
				RelationShip relation = new RelationShip();
				relation.setTimeRelation(new Date());
	...
	public List<User> getListMatchbyUser() {
		...
		List<RelationShip> listrelation = relationrepository
				.findRelationshipByUserIdAndStatusAndOnline(mentor.getUserId(), 2, true);
